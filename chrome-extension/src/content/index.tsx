...
/**
 * 防止 content script 被双重初始化：
 * - background onInstalled 主动注入 + Chrome 自动声明式注入同时发生时，
 *   两者可能触发同一帧的 module 重新执行，造成重复的事件监听器。
 * - 用 document.documentElement 属性作为跨 realm 标志（DOM 在所有 world 间共享）。
 * - unmount() 会清除该标志，允许 reload 后的新 context 重新初始化。
 */
const CROW_INIT_ATTR = 'data-crow-cs-init';
const alreadyInitialized = document.documentElement.hasAttribute(CROW_INIT_ATTR);
document.documentElement.setAttribute(CROW_INIT_ATTR, '');

let reactRoot: Root | null = null;

// 「连接插件」桥接监听——必须在所有页面（含 Crow 自身站点）上运行，
// 且不受去重 guard 限制（多次注入时也只需一个桥接监听，由 guard 保证）。
...
function unmount() {
  ...
  if (host) host.remove();
  // 清除初始化标志，允许后续新注入（如 onInstalled 重新注入）重新运行初始化
  document.documentElement.removeAttribute(CROW_INIT_ATTR);
}
