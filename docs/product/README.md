...
| 笔记分类（B-1） | [笔记分类](../../dev/active/笔记分类/) | 复用 `tags` 做主分类 + 笔记本筛选；文档已建档待开发 |
| 划词上下文（B-2） | [划词上下文](../../dev/active/划词上下文/) | 扩展附带选区前后文 `surroundingText`；与追问 `context` 分离 |
| 名词解释与消歧（C-1） | [名词解释与消歧](../../dev/active/名词解释与消歧/) | Prompt 增强：缩写全称与多义消歧 |
| 截图上传（C-2） | [截图上传](../../dev/active/截图上传/) | Web 粘贴/选图多模态解释（inline 压缩） |
| 划词保存重复笔记校验 | [划词保存重复笔记校验](../../dev/active/划词保存重复笔记校验/) | 扩展对齐 Web 重复检测（都保留/覆盖） |
| 链接内容抓取（D-1） | [链接内容抓取](../../dev/active/链接内容抓取/) | 用户确认后安全抓取 URL 正文；SSRF 优先 |
| 飞书等平台（D-2） | [飞书等平台](../../dev/active/飞书等平台/) | 先调研载体（网页扩展/机器人/插件）再决定是否实现 |
