...
| 笔记分类（B-1） | [笔记分类](../../dev/active/笔记分类/) | `notes.tags` + notes API/笔记本页筛选；可选 `PATCH` |
| 划词上下文（B-2） | [划词上下文](../../dev/active/划词上下文/) | 扩展选区 surrounding；`/api/explain` 新字段与 `prompts.ts` |
| 名词解释与消歧（C-1） | [名词解释与消歧](../../dev/active/名词解释与消歧/) | `lib/ai/prompts.ts` 规则增强；样例手测集 |
| 截图上传（C-2） | [截图上传](../../dev/active/截图上传/) | explain 多模态消息；体积校验；vision 模型 |
| 划词保存重复笔记校验 | [划词保存重复笔记校验](../../dev/active/划词保存重复笔记校验/) | 扩展 `ExplainCard` 对齐 Web normalize + 确认 UI |
| 链接内容抓取（D-1） | [链接内容抓取](../../dev/active/链接内容抓取/) | 新 `POST /api/fetch-url` + SSRF 防护库 |
| 飞书等平台（D-2） | [飞书等平台](../../dev/active/飞书等平台/) | 调研里程碑优先；实现期适配 `/api/explain` |
