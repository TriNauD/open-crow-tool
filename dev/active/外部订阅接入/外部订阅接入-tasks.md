...
- [ ] 在 Supabase SQL Editor 执行 DDL（建 subscribers 表）

## 验收标准

- [ ] 访问 `/subscribe`，填邮箱提交，Supabase Table Editor 里能看到新记录
- [ ] 手动触发 cron，该邮箱收到邮件，邮件底部有退订链接
- [ ] 点退订链接，DB 里 status 变 cancelled，下次 cron 不再发送
- [ ] 重复提交同一邮箱，返回"已订阅"而不报错
