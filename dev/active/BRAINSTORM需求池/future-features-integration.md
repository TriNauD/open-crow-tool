...
| 12 | 链接抓取成本护栏 | `fea/fetch-url-cost-guard` | ✅ | 随部署冒烟（限流/403 无 UI，curl 即可验证） | `/api/fetch-url` 按 IP 限流 20/h + Origin 校验 + URL≤2048；共用 `lib/request-guard.ts`（explain 同款护栏在 dev 侧 `bugfix/explain-cost-guard`，随 dev 合回 future） |
