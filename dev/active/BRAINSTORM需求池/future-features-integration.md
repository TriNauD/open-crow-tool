...
| 12 | 链接抓取成本护栏 | `fea/fetch-url-cost-guard` | ✅ | 随部署冒烟（限流/403 无 UI，curl 即可验证） | `/api/fetch-url` 按 IP 限流 20/h + Origin 校验 + URL≤2048；explain 同款护栏已随 [#32](https://github.com/TriNauD/open-crow-tool/pull/32) 进 dev 并合回 future（BF-1 见 `dev/logs/解释接口成本护栏-log.md`） |
